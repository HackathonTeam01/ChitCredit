...
export async function addContribution(
  ...
  try {
    ...
    const autoPaymentEvent = paid_on_time && amount_paid >= amount_due
      ? {
          type: "contribution_auto_paid",
          member_id,
          amount: amount_paid,
          source: "savings_wallet",
          message: `Your chit payment of Rs ${amount_paid} was auto-paid from your savings wallet on time.`,
        }
      : null;
    ...
    res.status(201).json({
      ...
      contribution,
      auto_payment_event: autoPaymentEvent,
    });
  } catch (error) {
    const message =
